...
// --- The headline verdict: live conformance against RFC 10032 vectors -----
...
test('conformance badge passes and its counters are internally consistent', async ({ page }) => {
  ...
  // "P/T official RFC 10032 vectors conform - E encryptions ..., F forgeries rejected."
  const counts = /(\d+)\/(\d+) official RFC 10032 vectors conform - (\d+) encryptions [^,]+, (\d+) forgeries rejected/.exec(
    summary,
  );
...
test('avalanche trace starts at one bit and reaches ~half the state', async ({ page }) => {
  ...
  // Seed state plus RFC 10032's 16 initialization updates.
