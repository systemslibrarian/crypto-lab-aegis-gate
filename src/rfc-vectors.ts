/**
 * Official AEGIS-256 test vectors from RFC 10032, Appendix A.3. These are
 * public, non-secret values whose only purpose is conformance testing — the
 * demo replays them in the browser so a visitor can watch this implementation
 * reproduce the RFC's bytes exactly.
 *
...
/**
 * Official must-reject vectors from RFC 10032, Appendix A.3 (Test Vectors
 * 6-9): a swapped key/nonce, a flipped ciphertext bit, altered AD, and a
 * flipped tag bit. A conformant implementation must refuse to decrypt every
 * one of them.
 *
 * Kept in sync with test/vectors/aegis-256-test-vectors.json by
 * test/conformance.test.ts.
